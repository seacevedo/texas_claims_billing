...
with inst_billing_header as (
    ...
    cast(ADMISSION_HOUR as time) as ADMISSION_HOUR, 
    to_date(cast(ADMISSION_DATE as varchar), 'yyyy-mm-dd hh:mi:ss.ff9') as ADMISSION_DATE,
    cast(ADMISSION_TYPE_CODE as char) as ADMISSION_TYPE_CODE, 
    cast(ADMITTING_DIAGNOSIS_CODE as varchar) as ADMITTING_DIAGNOSIS_CODE,
    cast(PRINCIPAL_DIAGNOSIS_CODE as varchar) as PRINCIPAL_DIAGNOSIS_CODE,
    ...
    from {{ source('texas_claims_src', 'INST_BILLING_HEADER') }}
)

select *, current_timestamp() as transformed_timestamp 
from inst_billing_header
